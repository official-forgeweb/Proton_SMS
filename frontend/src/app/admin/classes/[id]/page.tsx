...
export default async function ClassProfilePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const basePath = session.role === 'coordinator' ? '/coordinator' : '/admin';
         ...
         return (
             <DashboardLayout requiredRole={['admin', 'coordinator']}>
                 <div className="empty-state" style={{ padding: '80px 24px', textAlign: 'center' }}>
                     ...
                     <Link href={`${basePath}/classes`} className="btn btn-primary" style={{ textDecoration: 'none' }}>Back to Classes</Link>
                 </div>
             </DashboardLayout>
         );
    }
