...
export default function CoordinatorDashboardClient({ data: serverData }: CoordinatorDashboardClientProps) {
    const router = useRouter();
    const [data, setData] = useState<DashboardData | null>(serverData || null);
    const [loadingCharts, setLoadingCharts] = useState(!serverData?.charts?.fees?.length);
    const [loadingAlerts, setLoadingAlerts] = useState(!serverData?.alerts?.length);
        ...
        if (!serverData) {
                ...
                api.get('/dashboard/admin/charts')
                    .then(res => {
                        if (res.data && res.data.success) {
                            setData(prev => prev ? { ...prev, charts: res.data.data } : null);
                        }
                    })
                    .catch(console.error)
            ...
            if (loadingAlerts) {
                api.get('/dashboard/admin/alerts')
                    .then(res => {
                        if (res.data && res.data.success) {
                            setData(prev => prev ? { ...prev, alerts: res.data.data } : null);
                        }
                    })
                    .catch(console.error)
    ...
    }, [serverData, loadingCharts, loadingAlerts, fetchData]);
    ...
    // DB-Backed Historical Chart Series & Parity check
        ...
        return (
            <div style={{ padding: '32px', display: 'flex', flexDirection: 'column', gap: '32px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div className="skeleton" style={{ width: '400px', height: '40px', borderRadius: '12px' }} />
                </div>
        ...
        <div style={{ padding: '32px', margin: '0 auto', maxWidth: '1600px', display: 'flex', flexDirection: 'column', gap: '32px' }} className="animate-page-entry">
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '20px' }}>
                ...
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                    <QuickAction label="Add Student" icon={Plus} onClick={() => router.push('/coordinator/students')} colorClass="red" />
                    <QuickAction label="Add Fee" icon={DollarSign} onClick={() => router.push('/coordinator/fees')} colorClass="green" />
                    <QuickAction label="Create HET" icon={Award} onClick={() => router.push('/coordinator/hets')} colorClass="orange" />
                    <QuickAction label="Create Notice" icon={BookOpen} onClick={() => router.push('/coordinator/operations')} colorClass="indigo" />
                    <QuickAction label="View Enquiries" icon={Phone} onClick={() => router.push('/coordinator/enquiries')} colorClass="blue" />
                </div>
            </div>
                        ...
                        {alerts.length > 0 ? alerts.slice(0, 4).map((alert, idx) => {
                            ...
                            return (
                                <div 
                                    ...
                                    className={actionLink ? 'card-hover alert-card-transition' : ''}
                                >
