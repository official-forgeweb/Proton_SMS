    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const [calRes, statsRes] = await Promise.all([
                api.get('/attendance/calendar', { params: { student_id: studentId, start_date: startOfMonth, end_date: endOfMonth } }),
                api.get('/attendance/stats', { params: { student_id: studentId } })
            ]);
        ...
        return (
            <div style={{ padding: '40px', textAlign: 'center' }}>
                <style dangerouslySetInnerHTML={{__html: `
                    ...
                    .skeleton-cal { background: linear-gradient(90deg, #f0f0f5 25%, #e8e8f0 50%, #f0f0f5 75%); background-size: 200% 100%; animation: shimmer 4s infinite linear; border-radius: 16px; }
                `}} />
                <div className="skeleton-cal" style={{ height: '80px', marginBottom: '24px' }} />
                <div className="skeleton-cal" style={{ height: '400px' }} />
            </div>
        );
    }
    ...
    return (
        <div style={{ padding: '0', maxWidth: '1400px', margin: '0 auto' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '16px' }}>
                <div>
                    <h1 style={{ fontSize: '20px', fontWeight: 800, color: '#1A1D3B', letterSpacing: '-0.02em', margin: 0 }}>Academic Calendar</h1>
                    <p style={{ color: '#5E6278', fontSize: '13.5px', fontWeight: 500, margin: '4px 0 0 0' }}>Detailed schedule of classes and examinations.</p>
                </div>
                <div style={{ display: 'flex', gap: '8px', background: 'white', padding: '4px', borderRadius: '12px', border: '1px solid #E2E8F0', boxShadow: '0 4px 12px rgba(0,0,0,0.03)' }}>
                    ...
                    <div style={{ padding: '0 12px', display: 'flex', alignItems: 'center', fontWeight: 700, color: '#1A1D3B', minWidth: '130px', justifyContent: 'center', fontSize: '14px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '16px', marginBottom: '24px' }}>
                ...
                {stats?.subjects.slice(0, 2).map((s, idx) => (
                    <div key={idx} className="shadow-sm" style={{ 
                        padding: '18px 24px', 
                        borderRadius: '18px', 
