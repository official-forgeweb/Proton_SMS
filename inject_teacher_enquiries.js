...
if (!content.includes('const [queries, setQueries]')) {
    content = content.replace('const [homeworkHistory, setHomeworkHistory] = useState<any[]>([]);', 
                              'const [homeworkHistory, setHomeworkHistory] = useState<any[]>([]);\n    const [queries, setQueries] = useState<any[]>([]);');
}

// 3. Update fetchExtraReports
if (!content.includes('api.get(`/queries`')) {
...
`            const [attRes, testRes, perfRes, hwRes, queriesRes] = await Promise.allSettled([
                api.get(\`/students/\${params.id}/attendance\`),
                api.get(\`/students/\${params.id}/tests\`),
                api.get(\`/students/\${params.id}/performance\`),
                api.get(\`/students/\${params.id}/homework-history\`),
...
// 4. Add the tab (Teacher tabs might not have Fees, they have Remarks, Overview, Academics, Attendance)
if (!content.includes("{ id: 'enquiries', label: 'Enquiries'")) {
    content = content.replace(
...
if (!content.includes("activeTab === 'enquiries'")) {
    content = content.replace(
        '            </div>\n        </DashboardLayout>',
        enquiriesBlock + '\n            </div>\n        </DashboardLayout>'
    );
...
console.log("Teacher page updated successfully.");
