        ...
        filteredTests.forEach(test => {
            ...
            if (test.status === 'completed' || test.results_published) {
                ...
                if (test.results_published)
                    testStatus = 'result_published';
                if (result && result.was_present === false)
                    testStatus = 'missed';
            }
            calendarData[date].push({
                id: test.id,
...
router.get('/stats', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const pastSessions = await database_1.default.timetable.findMany({
            where: {
                class_id: { in: classIds },
                date: { lte: todayStr }
            },
        ...
        const validPastSessions = pastSessions.filter(session => {
            ...
            if (session.date === todayStr) {
                ...
                if (sessionTime > now)
                    return false; // Future session today
            }
            const enrolledSubjects = enrolledSubjectsByClass[session.class_id] || [];
            if (enrolledSubjects.length > 0) {
                return enrolledSubjects.includes(session.subject.trim().toLowerCase());
...
router.post('/mark', auth_1.authenticateToken, (0, auth_1.authorize)('admin', 'teacher'), async (req, res) => {
    try {
        ...
        if (req.user.role === 'teacher' || req.user.role === 'coordinator') {
            ...
            await logTeacherActivity(req.user.id, 'attendance_mark', null, JSON.stringify({
                timetable_id,
...
router.get('/session/:timetable_id', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const classEnrollments = await database_1.default.studentClassEnrollment.findMany({
