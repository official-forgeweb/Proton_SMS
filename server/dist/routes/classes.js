...
const syncClassSubjects = async (tx, classId, subjectIds) => {
...
router.get('/:id/subjects', auth_1.authenticateToken, async (req, res) => {
    try {
        const id = paramId(req);
        const mappings = await database_1.default.classSubject.findMany({
            where: { class_id: id },
            include: { subject: true }
        });
...
router.post('/bulk-create', auth_1.authenticateToken, (0, auth_1.authorize)('admin', 'coordinator'), async (req, res) => {
    try {
        ...
        const uniqueRawSubjects = new Set();
        ...
        const subjectResolutionMap = new Map();
        await Promise.all(Array.from(uniqueRawSubjects).map(async (sub) => {
        ...
        const result = await database_1.default.$transaction(async (tx) => {
            ...
            for (const item of classesToCreate) {
                ...
                if (canonicalSubjects.length > 0) {
                    ...
                    const subjectRecords = await Promise.all(canonicalSubjects.map(async (name) => {
                        const key = (0, normalization_1.getNormalizedKey)(name);
        ...
        if (schedule && Array.isArray(schedule) && schedule.length > 0) {
            ...
            for (const s of schedule) {
                ...
                if (subjectId && !isUUID(subjectId)) {
                    const canonicalName = await (0, normalization_1.resolveCanonicalSubject)(subjectId);
                    const key = (0, normalization_1.getNormalizedKey)(canonicalName);
                    let subjectRecord = await database_1.default.subject.findUnique({ where: { normalized_key: key } });
                    if (!subjectRecord) {
                        subjectRecord = await database_1.default.subject.create({
            ...
            if (resolvedSchedules.length > 0) {
                ...
                await syncClassSubjects(database_1.default, newClass.id, subjectIds);
            }
        }
        ...
        if (schedule && Array.isArray(schedule)) {
            ...
            if (schedule.length > 0) {
                ...
                if (resolvedSchedules.length > 0) {
                    ...
                    await syncClassSubjects(database_1.default, id, subjectIds);
                }
            }
            else {
                await syncClassSubjects(database_1.default, id, []);
            }
        }
        const result = await database_1.default.class.findUnique({
            where: { id },
