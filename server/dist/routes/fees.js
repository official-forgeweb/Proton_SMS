...
const generateReceiptNumber = () => `RCPT${new Date().getFullYear()}${String(Math.floor(100000 + Math.random() * 900000))}`;
// ──────────────────────────────────────────────
...
// ──────────────────────────────────────────────
async function reallocatePaymentsAndRecalculate(assignmentId, tx) {
    ...
    const installments = assignment.installments.map((inst) => ({
        ...inst,
        paid_amount: 0,
    ...
    const totalPaid = Math.round(installments.reduce((sum, inst) => sum + inst.paid_amount, 0) * 100) / 100;
    const totalPending = Math.round(installments.reduce((sum, inst) => sum + inst.remaining_amount, 0) * 100) / 100;
...
router.post('/assignments/preview', auth_1.authenticateToken, (0, auth_1.authorize)('admin', 'coordinator'), async (req, res) => {
    try {
...
router.post('/assignments', auth_1.authenticateToken, (0, auth_1.authorize)('admin', 'coordinator'), async (req, res) => {
    try {
        const { student_id, fee_structure_id, academic_year, total_fee, admission_fee, registration_fee, discount_percentage, discount_amount, scholarship_amount, payment_mode, installment_strategy, installment_count, course_duration_months, preferred_due_day, start_date, notes, custom_installments // Array of { installment_number, amount, due_date }
         } = req.body;
        const existing = await database_1.default.studentFeeAssignment.findFirst({
            where: { student_id }
        });
        if (existing) {
        ...
        const result = await database_1.default.$transaction(async (tx) => {
            ...
            if (payment_mode === 'one_time') {
                ...
                const sum = custom_installments.reduce((acc, curr) => acc + Number(curr.amount), 0);
                if (Math.abs(sum - final_fee) > 0.01) {
                    throw new Error(`Sum of custom installments (${sum}) does not match net payable fee (${final_fee})`);
                }
                generatedInstallments = custom_installments.map((inst, idx) => ({
            ...
            await Promise.all(generatedInstallments.map((inst) => tx.feeInstallment.create({
                data: {
...
router.get('/assignments/:id', auth_1.authenticateToken, (0, auth_1.authorize)('admin', 'coordinator'), async (req, res) => {
    try {
...
// POST /api/fees/pay
router.post('/pay', auth_1.authenticateToken, (0, auth_1.authorize)('admin', 'coordinator'), async (req, res) => {
    try {
        const { student_id, amount_paid, payment_method, transaction_id, remarks } = req.body;
        const assignment = await database_1.default.studentFeeAssignment.findFirst({
            where: { student_id }
        });
        if (!assignment) {
            res.status(404).json({ success: false, message: 'Fee assignment not found' });
...
router.get('/payments', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        if (studentIdQuery && isUUID(studentIdQuery))
            where.student_id = studentIdQuery;
        // Students can only see their own payments
        if (req.user.role === 'student') {
            const studentUser = await database_1.default.student.findUnique({
...
router.delete('/payments/:id', auth_1.authenticateToken, (0, auth_1.authorize)('admin', 'coordinator'), async (req, res) => {
    try {
        const paymentId = req.params.id;
        const { remarks } = req.body || {};
        const payment = await database_1.default.feePayment.findUnique({
            where: { id: paymentId },
            include: {
                student: {
                    include: {
...
router.get('/stats', auth_1.authenticateToken, (0, auth_1.authorize)('admin', 'coordinator'), async (req, res) => {
    try {
        const [paymentAgg, assignments] = await Promise.all([
            ...
            database_1.default.studentFeeAssignment.findMany({
                select: {
                    payment_status: true,
                    total_pending: true,
                }
        ...
        assignments.forEach(a => {
            ...
            if (a.payment_status === 'paid')
                paidStudents++;
            else if (a.payment_status === 'partial')
                partialStudents++;
...
router.post('/overdue-check', auth_1.authenticateToken, (0, auth_1.authorize)('admin', 'coordinator'), async (req, res) => {
    try {
        const assignments = await database_1.default.studentFeeAssignment.findMany({
            select: { id: true }
        });
