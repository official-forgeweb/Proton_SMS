...
const generateQueryNumber = () => `QRY${Date.now().toString(36).toUpperCase()}${Math.floor(Math.random() * 1000).toString().padStart(3, '0')}`;
// GET /api/queries — list queries based on role
router.get('/', auth_1.authenticateToken, async (req, res) => {
    try {
        const { status, query_type, student_id, raised_by, page = '1', limit = '50' } = req.query;
        ...
        if (req.user.role === 'student') {
            ...
            const assignedClasses = await database_1.default.class.findMany({
            ...
            if (student_id) {
                ...
                const hasTargeted = await database_1.default.studentQuery.count({
        ...
        const [total, queries] = await Promise.all([
            database_1.default.studentQuery.count({ where }),
...
router.get('/stats', auth_1.authenticateToken, (0, auth_1.authorize)('admin', 'coordinator', 'teacher'), async (req, res) => {
    try {
        const [total, newCount, processing, resolved, unresolved] = await Promise.all([
            database_1.default.studentQuery.count(),
            database_1.default.studentQuery.count({ where: { status: 'new' } }),
            database_1.default.studentQuery.count({ where: { status: 'processing' } }),
            database_1.default.studentQuery.count({ where: { status: 'resolved' } }),
            database_1.default.studentQuery.count({ where: { status: 'unresolved' } }),
        ]);
        // Type breakdown
        const typeBreakdown = await database_1.default.studentQuery.groupBy({
            by: ['query_type'],
            _count: true,
        });
        res.json({
...
router.get('/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        const query = await database_1.default.studentQuery.findUnique({
        ...
        if (req.user.role === 'student') {
            ...
            const assignedClassIds = assignedClasses.map(c => c.id);
            const isEnrolled = await database_1.default.studentClassEnrollment.count({
...
router.post('/', auth_1.authenticateToken, (0, auth_1.authorize)('admin', 'coordinator', 'teacher', 'student'), async (req, res) => {
    try {
        ...
        const student = await database_1.default.student.findUnique({ where: { id: finalStudentId } });
        if (!student) {
            res.status(404).json({ success: false, message: 'Student not found' });
            return;
        }
        const query = await database_1.default.studentQuery.create({
            data: {
        ...
        if (target_teacher_id) {
            const teacher = await database_1.default.teacher.findUnique({ where: { id: target_teacher_id }, select: { user_id: true } });
            if (teacher && teacher.user_id) {
                await (0, notifications_1.sendNotification)([teacher.user_id], req.user.id, 'general', 'New Student Query', `${query.student.first_name || ''} ${query.student.last_name || ''}`.trim() + ` raised a query: ${query_type}`, query.id);
            }
        }
...
router.put('/:id', auth_1.authenticateToken, (0, auth_1.authorize)('admin', 'coordinator', 'teacher'), async (req, res) => {
    try {
        ...
        const existing = await database_1.default.studentQuery.findUnique({ where: { id } });
        if (!existing) {
            res.status(404).json({ success: false, message: 'Query not found' });
            return;
        }
        ...
        const query = await database_1.default.studentQuery.update({
            where: { id },
            data: updateData,
            include: {
        ...
        if (query.student && query.student.user_id) {
            await (0, notifications_1.sendNotification)([query.student.user_id], req.user.id, 'general', 'Query Status Updated', `Your query ${query.query_number} status has been updated to: ${query.status}`, query.id);
        }
        res.json({ success: true, data: query });
    }
    catch (error) {
...
router.delete('/:id', auth_1.authenticateToken, (0, auth_1.authorize)('admin', 'coordinator'), async (req, res) => {
    try {
        await database_1.default.studentQuery.delete({ where: { id: req.params.id } });
...
router.get('/:id/details', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        // Fetch replies
        const replies = await database_1.default.queryReply.findMany({
            where: { query_id: id },
        ...
        // Fetch attachments
        const attachments = await database_1.default.queryAttachment.findMany({
            where: { query_id: id },
            orderBy: { created_at: 'asc' }
        });
        // Fetch audit logs
        const auditLogs = await database_1.default.queryAuditLog.findMany({
            where: { query_id: id },
            orderBy: { created_at: 'desc' },
            include: {
        ...
        if (req.user.role === 'admin' || req.user.role === 'teacher') {
            internalNotes = await database_1.default.queryInternalNote.findMany({
                where: { query_id: id },
                orderBy: { created_at: 'desc' },
                include: {
                    user: {
...
router.post('/:id/replies', auth_1.authenticateToken, async (req, res) => {
    try {
        const id = req.params.id;
        ...
        const reply = await database_1.default.queryReply.create({
            data: {
                query_id: id,
                user_id: req.user.id,
        ...
        // Create Audit Log
        await database_1.default.queryAuditLog.create({
            data: {
                query_id: id,
                user_id: req.user.id,
                action: 'reply_added',
        ...
        if (req.user.role === 'student') {
            ...
            if (query.target_teacher_id) {
                const teacher = await database_1.default.teacher.findUnique({ where: { id: query.target_teacher_id }, select: { user_id: true } });
                if (teacher && teacher.user_id)
                    notifyUserIds.push(teacher.user_id);
            }
            const admins = await database_1.default.user.findMany({ where: { role: 'admin', is_active: true }, select: { id: true } });
            admins.forEach(a => notifyUserIds.push(a.id));
            if (notifyUserIds.length > 0) {
                await (0, notifications_1.sendNotification)(notifyUserIds, req.user.id, 'general', 'New Reply from Student', `Student replied to query ${query.query_number}`, query.id);
            }
        }
        else {
            if (query.student && query.student.user_id) {
                await (0, notifications_1.sendNotification)([query.student.user_id], req.user.id, 'general', 'New Reply to your Query', `Support team replied to query ${query.query_number}`, query.id);
            }
        }
...
router.post('/:id/notes', auth_1.authenticateToken, (0, auth_1.authorize)('admin', 'coordinator', 'teacher'), async (req, res) => {
    try {
        const id = req.params.id;
        const { note } = req.body;
        if (!note || !note.trim()) {
        ...
        const query = await database_1.default.studentQuery.findUnique({ where: { id } });
        if (!query) {
            res.status(404).json({ success: false, message: 'Query not found' });
            return;
        }
        const internalNote = await database_1.default.queryInternalNote.create({
            data: {
                query_id: id,
                user_id: req.user.id,
                note: note.trim()
...
router.put('/:id/notes/:noteId', auth_1.authenticateToken, (0, auth_1.authorize)('admin', 'coordinator'), async (req, res) => {
    try {
        ...
        const existingNote = await database_1.default.queryInternalNote.findUnique({ where: { id: noteId } });
        if (!existingNote) {
            res.status(404).json({ success: false, message: 'Note not found' });
            return;
        }
        const updatedNote = await database_1.default.queryInternalNote.update({
            where: { id: noteId },
            data: { note: note.trim() },
            include: {
                user: {
...
router.delete('/:id/notes/:noteId', auth_1.authenticateToken, (0, auth_1.authorize)('admin', 'coordinator'), async (req, res) => {
    try {
        const noteId = req.params.noteId;
        const existingNote = await database_1.default.queryInternalNote.findUnique({ where: { id: noteId } });
        if (!existingNote) {
        ...
        await database_1.default.queryInternalNote.delete({ where: { id: noteId } });
        // Create Audit Log
        await database_1.default.queryAuditLog.create({
            data: {
                query_id: existingNote.query_id,
...
router.put('/:id/crm', auth_1.authenticateToken, (0, auth_1.authorize)('admin', 'coordinator', 'teacher'), async (req, res) => {
    try {
        const id = req.params.id;
        ...
        if (target_teacher_id !== undefined && target_teacher_id !== query.target_teacher_id) {
            ...
            if (target_teacher_id) {
                const teacher = await database_1.default.teacher.findUnique({ where: { id: target_teacher_id } });
                if (teacher) {
        ...
        const updatedQuery = await database_1.default.studentQuery.update({
            where: { id },
            data: updateData,
            include: {
                student: { select: { first_name: true, last_name: true, PRO_ID: true } },
        ...
        if (status && query.student && query.student.user_id) {
            await (0, notifications_1.sendNotification)([query.student.user_id], req.user.id, 'general', 'Query Updated', `Your query ${query.query_number} has been updated: ${auditLogsData.map(l => l.details).join(', ')}`, id);
        }
...
router.post('/:id/attachments', auth_1.authenticateToken, async (req, res) => {
    try {
        const id = req.params.id;
        ...
        const attachment = await database_1.default.queryAttachment.create({
            data: {
                query_id: id,
                file_name,
                file_url,
