...
class GoogleSheetsService {
    static getAuthClient() {
        ...
        if (!clientEmail || !privateKey) {
            throw new Error('Missing Google Sheets API credentials (GOOGLE_CLIENT_EMAIL, GOOGLE_PRIVATE_KEY) in server .env');
        }
     ...
     */
    static async readSpreadsheet(spreadsheetId, sheetName, availableSheetTitles) {
        if (!spreadsheetId) {
        ...
        if (sheetTitles.length > 0) {
            if (!sheetTitles.includes(sheetName)) {
                ...
                const caseInsensitiveMatch = sheetTitles.find((t) => t.toLowerCase() === sheetName.toLowerCase());
                if (caseInsensitiveMatch) {
                    ...
                    const partialMatch = sheetTitles.find((t) => t.toLowerCase().includes('video') ||
        ...
        const parsedRecords = dataRows
            ...
            return Object.entries(record).some(([key, value]) => key !== '__sheet_row_num' && value !== '');
        });
     ...
     */
    static async getSpreadsheetTabs(spreadsheetId) {
        if (!spreadsheetId)
            return [];
        try {
            const auth = this.getAuthClient();
            ...
            return availableSheets
                .map((s) => s.properties?.title)
                .filter((t) => !!t && t.toUpperCase() !== 'SALARY');
        }
        catch (err) {
     ...
     */
    static async testConnection(spreadsheetId) {
        try {
            const auth = this.getAuthClient();
            const sheets = googleapis_1.google.sheets({ version: 'v4', auth });
